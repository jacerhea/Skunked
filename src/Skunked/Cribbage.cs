...
public class Cribbage
{
    ...
    public void CutCard(CutCardCommand command)
    {
        ...
        if (State.OpeningRound.Complete)
        {
            ...
            _deck.Shuffle();
            Emit(new DeckShuffledEvent(State.Id, NewVersion, _deck.ToList()));
            var dealSize = State.GameRules.GetDealSize(State.PlayerIds.Count);
            var playerHands = _dealer.Deal(_deck, State.PlayerIds, State.PlayerIds.NextOf(command.PlayerId), dealSize); // get next player from who won cut.
    ...
    public void CountCrib(CountCribCommand command)
    {
        ...
        _deck.Shuffle(3);
        Emit(new DeckShuffledEvent(State.Id, NewVersion, _deck.ToList()));
        var startingWith = State.PlayerIds.NextOf(State.PlayerIds.NextOf(command.PlayerId));
        var dealSize = State.GameRules.GetDealSize(State.PlayerIds.Count);
        var playerHands = _dealer.Deal(_deck, State.PlayerIds, startingWith, dealSize);
        Emit(new HandsDealtEvent(State.Id, NewVersion, playerHands));
    }
